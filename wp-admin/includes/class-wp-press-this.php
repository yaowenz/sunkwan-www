...
class WP_Press_This
{
	...
	public function site_settings ()
	{
		 ...
		 *       
		 * @param bool $redirect
		 *        	Whether to redirect in parent window or not. Default false.
		 */
	 ...
	 *        	Optional. Current expected markup for Press This. Expects slashed. Default empty.
	...
	public function save_post ()
	{
		...
		else
		{
			 ...
			 *       
			 * @param string $url
			 *        	Redirect URL. If `$status` is 'publish', this will be the post permalink.
			 ...
			 *        	Post status.
			 */
			$redirect = apply_filters('press_this_save_redirect', $redirect, $post_id, 
	...
	public function add_category ()
	{
		...
		foreach ($names as $cat_name)
		{
			...
			if (term_exists($cat_name, $taxonomy->name, $parent))
			{
				...
				{
					wp_send_json_error(
					array('errorMessage' => __('This category already exists.')));
				}
				else
				{
					continue;
		...
		foreach ($added as $new_cat_id)
		{
			...
			if (is_wp_error($new_cat))
			{
				wp_send_json_error(
				array(
				'errorMessage' => __('Error while adding the category. Please try again later.')));
			}
			$data[] = array('term_id' => $new_cat->term_id, 'name' => $new_cat->name, 
	...
	public function fetch_source_html ($url)
	{
		...
		'user-agent' => 'Press This (WordPress/' . get_bloginfo('version') . '); ' .
		 get_bloginfo('url')));
		...
		'link' => array('rel' => true, 'itemprop' => true, 'href' => true), 
		'meta' => array('property' => true, 'name' => true, 'content' => true));
	 ...
	 *       
	 * @param string $url
	 *        	URL to check for length and validity.
	...
	private function _limit_embed ($src)
	{
		...
		if (preg_match('!//(m|www)\.youtube\.com/(embed|v)/([^?]+)\?.+$!i', $src, $src_matches))
		{
			...
			if (preg_match('!//player\.vimeo\.com/video/([\d]+)([?/].*)?$!i', $src, $src_matches))
			{
				...
				if (preg_match('!//vimeo\.com/moogaloop\.swf\?clip_id=([\d]+)$!i', $src, 
				$src_matches))
				{
					...
					if (preg_match('!//vine\.co/v/([^/]+)/embed!i', $src, $src_matches))
					{
						...
						if (preg_match(
						'!//(www\.)?dailymotion\.com/embed/video/([^/?]+)([/?].+)?!i', $src, 
						$src_matches))
						{
	 ...
	 *        	Meta value.
	 * @param array $data
	 *        	Associative array of source data.
	 ...
	 *        	URL to scan.
	 * @param array $data
	 *        	Optional. Existing data array if you have one. Default empty array.
	...
	public function source_data_fetch_fallback ($url, $data = array())
	{
		...
		if (preg_match_all('/<meta [^>]+>/', $source_content, $matches))
		{
			...
			{
				if (preg_match('/(property|name)="([^"]+)"[^>]+content="([^"]+)"/', $value, 
				$new_matches))
				{
		...
		if (preg_match_all('/<img [^>]+>/', $source_content, $matches))
		{
			...
			foreach ($items as $value)
			{
				if ((preg_match('/width=(\'|")(\d+)\\1/i', $value, $new_matches) &&
				 $new_matches[2] < 256) ||
				 (preg_match('/height=(\'|")(\d+)\\1/i', $value, $new_matches) &&
				 $new_matches[2] < 128))
				{
					continue;
				}
		...
		if (preg_match_all('/<link [^>]+>/', $source_content, $matches))
		{
			...
			foreach ($items as $value)
			{
				if (preg_match('/rel=["\'](canonical|shortlink|icon)["\']/i', $value, $matches_rel) &&
	...
	public function merge_or_fetch_data ()
	{
		 ...
		 *       
		 * @param bool $enable
		 *        	Whether to enable media discovery.
		 */
		if (apply_filters('enable_press_this_media_discovery', true))
		{
			...
			if (! empty($_REQUEST['i']) &&
			 ($img_src = $this->_limit_img(wp_unslash($_REQUEST['i']))))
			{
				if (empty($data['_images']))
				{
...
<div id="post-formats-select">
	<fieldset>
		...
		<label for="post-format-0"
			...
			value="<?php echo $attr_format; ?>"
			<?php checked( $post_format, $format ); ?> /> <label
			for="post-format-<?php echo $attr_format ?>"
...
<?php
	...
	public function categories_html ($post)
	{
		...
		{
			?>
<button type="button" class="add-cat-toggle button-link"
	aria-expanded="false">
...
<div class="add-category is-hidden">
	...
	<input type="text" id="new-category" class="add-category-name"
		placeholder="<?php echo esc_attr( $taxonomy->labels->new_item_name ); ?>"
		value="" aria-required="true"> <label class="screen-reader-text"
	...
	<div class="postform-wrapper">
			...
			'name' => 'new-category-parent', 'orderby' => 'name', 'hierarchical' => 1, 
...
<div class="categories-search-wrapper">
	<input id="categories-search" type="search" class="categories-search"
		placeholder="<?php esc_attr_e( 'Search categories by name' ) ?>"> <label
		for="categories-search"> <span class="dashicons dashicons-search"></span><span
...
<button type="button" class="button-link tagcloud-link"
	id="link-post_tag" aria-expanded="false"><?php echo $taxonomy->labels->choose_from_most_used; ?></button>
<?php
		}
	}
	/**
	...
	public function get_images ($data)
	{
		...
		if (! empty($data['_images']))
		{
			foreach ($data['_images'] as $src)
			{
				...
				{
					$src = preg_replace('%http://[\d]+\.gravatar\.com/%', 
					'https://secure.gravatar.com/', $src);
				}
				$prot_relative_src = preg_replace('/^https?:/', '', $src);
	...
	public function get_suggested_content ($data)
	{
		 ...
		 * @since 4.2.0
		 *       
		 * @param array $default_html
		 *        	Associative array with three possible keys:
	...
	public function html ()
	{
		...
		$site_data = array('v' => ! empty($data['v']) ? $data['v'] : '', 
...
<script type="text/javascript">
	...
	<?php
		...
		$show_categories = current_user_can($categories_tax->cap->assign_terms) ||
		 current_user_can($categories_tax->cap->edit_terms);
...
<?php
		...
		$admin_body_class .= ' branch-' .
		 str_replace(array('.', ','), '-', floatval($wp_version));
		$admin_body_class .= ' version-' .
		 str_replace('.', '-', preg_replace('/^([.0-9]+).*/', '$1', $wp_version));
		$admin_body_class .= ' admin-color-' .
		 sanitize_html_class(get_user_option('admin_color'), 'fresh');
		$admin_body_class .= ' locale-' .
		...
		?>
<body
	class="wp-admin wp-core-ui <?php echo $admin_body_classes . ' ' . $admin_body_class; ?>">
	<div id="adminbar" class="adminbar">
		<h1 id="current-site" class="current-site">
			<a class="current-site-link"
				href="<?php echo esc_url( home_url( '/' ) ); ?>" target="_blank"
				rel="home"> <span class="dashicons dashicons-wordpress"></span> <span
				class="current-site-name"><?php bloginfo( 'name' ); ?></span>
			</a>
		</h1>
		<button type="button" class="options button-link closed">
			<span class="dashicons dashicons-tag on-closed"></span> <span
				class="screen-reader-text on-closed"><?php _e( 'Show post options' ); ?></span>
	...
	<div id="scanbar" class="scan">
		<form method="GET">
			...
			<input type="url" name="u" id="url-scan" class="scan-url"
				...
				placeholder="<?php esc_attr_e( 'Enter a URL to scan' ) ?>" /> <input
				type="submit" name="url-scan-submit" id="url-scan-submit"
				class="scan-submit" value="<?php esc_attr_e( 'Scan' ) ?>" />
			<?php wp_nonce_field( 'scan-site' ); ?>
		</form>
	</div>

	<form id="pressthis-form" method="post" action="post.php"
		autocomplete="off">
		<input type="hidden" name="post_ID" id="post_ID"
			value="<?php echo $post_ID; ?>" /> <input type="hidden" name="action"
			value="press-this-save-post" /> <input type="hidden"
			name="post_status" id="post_status" value="draft" /> <input
			type="hidden" name="wp-preview" id="wp-preview" value="" /> <input
			type="hidden" name="post_title" id="post_title" value="" /> <input
			type="hidden" name="pt-force-redirect" id="pt-force-redirect"
			value="" />
		<?php
	...
	<div class="wrapper">
		...
		?>
				...
				<div id="app-container" class="editor">
					...
					<h2 id="title-container" class="post-title" contenteditable="true"
						spellcheck="true"
						aria-label="<?php esc_attr_e( 'Post title' ); ?>" tabindex="0"><?php echo esc_html( $post_title ); ?></h2>
		...
		array('drag_drop_upload' => true, 'editor_height' => 600, 'media_buttons' => false, 
		'textarea_name' => 'post_content', 'teeny' => true, 
		'tinymce' => array('resize' => false, 'wordpress_adv_hidden' => false, 
		'add_unload_trigger' => false, 'statusbar' => false, 'autoresize_min_height' => 600, 
		...
		'toolbar2' => 'undo,redo'), 
		'quicktags' => array('buttons' => 'strong,em,link,block,del,ins,img,ul,ol,li,code,more')));
		?>
			</div>
			</div>
			...
			<div class="options-panel is-off-screen is-hidden" tabindex="-1">
				...
				<?php if ( $supports_formats ) : ?>
					<button type="button" class="post-option">
						<span class="dashicons dashicons-admin-post"></span> <span
							class="post-option-title"><?php _ex( 'Format', 'post format' ); ?></span>
				...
				<?php if ( $show_categories ) : ?>
					<button type="button" class="post-option">
						<span class="dashicons dashicons-category"></span> <span
							class="post-option-title"><?php _e( 'Categories' ); ?></span> <span
							class="dashicons post-option-forward"></span>
					</button>
				<?php endif; ?>

				<?php if ( $show_tags ) : ?>
					<button type="button" class="post-option">
						<span class="dashicons dashicons-tag"></span> <span
							class="post-option-title"><?php _e( 'Tags' ); ?></span> <span
							class="dashicons post-option-forward"></span>
					</button>
				<?php endif; ?>
			</div>
			...
			<?php if ( $supports_formats ) : ?>
				<div class="setting-modal is-off-screen is-hidden">
					<button type="button" class="modal-close">
						<span class="dashicons post-option-back"></span> <span
							class="setting-title" aria-hidden="true"><?php _ex( 'Format', 'post format' ); ?></span>
			...
			<?php if ( $show_categories ) : ?>
				<div class="setting-modal is-off-screen is-hidden">
					<button type="button" class="modal-close">
						<span class="dashicons post-option-back"></span> <span
							class="setting-title" aria-hidden="true"><?php _e( 'Categories' ); ?></span>
						<span class="screen-reader-text"><?php _e( 'Back to post options' ) ?></span>
					</button>
			...
			<?php if ( $show_tags ) : ?>
				<div class="setting-modal tags is-off-screen is-hidden">
					<button type="button" class="modal-close">
						<span class="dashicons post-option-back"></span> <span
							class="setting-title" aria-hidden="true"><?php _e( 'Tags' ); ?></span>
						<span class="screen-reader-text"><?php _e( 'Back to post options' ) ?></span>
					</button>
		...
		<div class="press-this-actions">
			<div class="pressthis-media-buttons">
				<button type="button" class="insert-media" data-editor="pressthis">
					<span class="dashicons dashicons-admin-media"></span> <span
						class="screen-reader-text"><?php _e( 'Add Media' ); ?></span>
				</button>
			</div>
				...
				<div class="split-button">
					<div class="split-button-head">
						...
						<button type="button" class="split-button-toggle"
							...
							<i class="dashicons dashicons-arrow-down-alt2"></i> <span
								class="screen-reader-text"><?php _e('More actions'); ?></span>
						</button>
					</div>
					<ul class="split-button-body">
						<li><button type="button"
								class="button-link draft-button split-button-option"><?php _e( 'Save Draft' ); ?></button></li>
						<li><button type="button"
								class="button-link standard-editor-button split-button-option"><?php _e( 'Standard Editor' ); ?></button></li>
						<li><button type="button"
								class="button-link preview-button split-button-option"><?php _e( 'Preview' ); ?></button></li>
					</ul>
				</div>
			</div>
		</div>
